 ...
 */
 ...
 class Blockchain {
      ...
      * You will need to check for the height to assign the `previousBlockHash` (if statement),
     ...
     _addBlock(block) {
         ...
         return new Promise(async (resolve, reject) => {
             ...
             //get timestamp
             blockVar.time = new Date().getTime().toString().slice(0,-3);
             if(height >= 0){
                 blockVar.height = height + 1;
                 let previousBlock = self.chain[self.height];
                 blockVar.previousBlockHash = previousBlock.hash;
                 // Verify signature
                 blockVar.hash = SHA256(JSON.stringify(blockVar)).toString();
                 self.chain.push(blockVar);
